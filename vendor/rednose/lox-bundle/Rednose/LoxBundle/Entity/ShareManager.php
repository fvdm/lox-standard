...
class ShareManager
{
    ...
    public function __construct(EventDispatcherInterface $dispatcher, EntityManager $em)
    {
        ...
        $this->em         = $em;
        $this->repository = $em->getRepository('Rednose\LoxBundle\Entity\Share');
    }
    ...
    public function removeShare(Share $share)
    {
        ...
        $pointers = $this->em->getRepository('Rednose\LoxBundle\Entity\Item')->findBy(array(
    ...
    public function synchronizeInvites(Share $share)
    {
        ...
        // Current users
        $currentInvites = $this->em->getRepository('Rednose\LoxBundle\Entity\Invitation')->findByShare($share);
        $currentUsers = array();
    ...
    public function removeInvitation(Invitation $invite)
    {
        ...
        $pointer = $this->em->getRepository('Rednose\LoxBundle\Entity\Item')->findOneBy(array(
            'shareOf' => $share->getItem(),
    ...
    {
        return $this->em->getRepository('Rednose\LoxBundle\Entity\Invitation')->findOneById($id);
    }

    public function findInvitationByItem(User $user, Item $item)
    {
        return $this->em->getRepository('Rednose\LoxBundle\Entity\Invitation')->findOneBy(array(
            'receiver' => $user,
            'item'     => $item,
        ));
    }
    ...
    public function findInvitationsForUser(User $user)
    {
        ...
        $results = $this->em->getRepository('Rednose\LoxBundle\Entity\Invitation')->findBy(
            array('receiver' => $user, 'state' => Invitation::STATE_PENDING),
            array('createdAt' => 'DESC')
        );

        $results = array_merge($results, $this->em->getRepository('Rednose\LoxBundle\Entity\Invitation')->findBy(
            array('receiver' => $user, 'state' => Invitation::STATE_ACCEPTED),
            array('createdAt' => 'DESC')
        ));

        return array_merge($results, $this->em->getRepository('Rednose\LoxBundle\Entity\Invitation')->findBy(
    ...
    {
        $pending = $this->em->getRepository('Rednose\LoxBundle\Entity\Invitation')->findBy(array(
            'receiver' => $user,
    ...
    {
        $invite = $this->em->getRepository('Rednose\LoxBundle\Entity\Invitation')->findOneBy(array(
            'receiver' => $receiver,
            'share'    => $share,
        ));
